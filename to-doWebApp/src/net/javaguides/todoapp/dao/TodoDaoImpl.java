...
public class TodoDaoImpl implements TodoDao {
	...
	private static final String UPDATE_TODO = "update todos set title = ?, description =?, target_date =?, is_done = ? where username = ?;";
	...
	@Override
	public List<Todo> selectUserTodos(String usernameString) {
		String q = "select id,title,username,description,target_date,is_done from todos where username=?;";
		Todo todo = new Todo();
		List<Todo> todoList = new LinkedList<Todo>();
		try(Connection connection = JDBCUtils.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(q);) {
	...
	public static void main(String[] args) {
		...
		List<Todo> l = dao.selectUserTodos("c2p19");
		System.out.println("c2p: " + l);
		for (Todo todo : l) {
